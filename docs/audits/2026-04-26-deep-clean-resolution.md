...
**Commits:** 30 (29 plan tasks + Task 28 pre-flight + Task 18 host-verification fix)
**Tests:** 170 passing across 14 files (was 50 across 3 before this work).
...
## Manual host verification — completed (UI/runtime)

Verified locally on 2026-04-27 against the host's real Turso DB via Chrome DevTools MCP:

| Source task | Result                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Task 10     | ✅ Clicking a draft row routes to `/draft/<id>`. No `?draft=` references remain in `src/`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Task 17     | ✅ Draft detail renders mana costs (W/U/R/G/B/C and multi-color like WU, BR) and GIH WR for picked card and all expanded alternatives.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Task 18     | ❌→✅ **Found and fixed a regression.** The plan's Task 18 commit added `cardImageUrl()` but never wired `image_url` through `CardLink` props. `CardData` did not include `imageUrl`, the JOIN didn't `SELECT c.image_url`, and `CardLink` only received `{ name }` — so hovers always hit `api.scryfall.com/cards/named` (302) → `cards.scryfall.io` (200) regardless of whether the cached URL was populated. Fixed in commit `d90e6d5`: `CardData.imageUrl` added, `getDraftWithCardData` selects `c.image_url`, `CardLink` accepts an optional `imageUrl` prop, `PickRow` forwards it. Re-verified via DevTools — hover now produces a single direct request to `cards.scryfall.io/.../<id>.jpg` with no API redirect. New test: `queries.test.ts::"surfaces cards.image_url alongside stats"`. Chat-side hovers (markdown `card:` links) still fall back to the API URL because there's no per-card data plumbed into Chat — out of scope for this fix. |
| Task 19     | ✅ Instrumented `PickRowImpl` with `console.count` per render. After mount, clicking one pick row produces exactly one row's render counts (StrictMode dev double-invocation), with no re-renders for the other 44 sibling rows. Verified twice with two different rows. Instrumentation reverted.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Task 21     | ✅ `Chat.tsx::generateMessageId` uses `\`msg-${crypto.randomUUID()}\``. Verified `crypto.randomUUID` is available in the dev runtime; sample output matches `/^msg-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/`. Audited `Date.now()`usage in`src/app/`: only timestamps and rate-limiter math, no other id generators.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |

## Manual host verification — still deferred (sync/infra)

These need real network access and cannot be checked against a local dev server alone:
